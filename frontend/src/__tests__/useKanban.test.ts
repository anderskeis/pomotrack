...
describe('useKanban', () => {
    ...
    it('increments pomodoros on active task', () => {
        const { todoTasks, addTask, moveTask, incrementActiveTaskPomodoros, activeTask } = useKanban()
        addTask('Task 1')
        const id = todoTasks.value[0].id
    ...
    it('clears done tasks', () => {
        const { tasks, todoTasks, doneTasks, addTask, moveTask, clearDoneTasks } = useKanban()
        addTask('Task 1')
        addTask('Task 2')
